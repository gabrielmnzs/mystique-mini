...
export interface TokenDictionary {
  ...
  getVar(path: string | readonly string[], fallback?: string): string | undefined
...
function sortConditions(conditions: Iterable<[string, TokenPrimitive]>): Array<[string, TokenPrimitive]> {
...
function compareConditions(left: string | undefined, right: string | undefined): number {
...
function sortedBreakpointNames(breakpoints: Readonly<Record<string, string>> | undefined): string[] {
  if (!breakpoints) return []
  const toPixels = (value: string) => {
    const match = value.trim().match(/^(-?\d*\.?\d+)\s*(px|em|rem)?$/i)
    if (!match) return Number.POSITIVE_INFINITY
    const amount = Number(match[1])
    return match[2]?.toLowerCase() === 'px' || match[2] === undefined ? amount : amount * 16
  }
  return Object.entries(breakpoints)
    .sort(([leftName, left], [rightName, right]) => toPixels(left) - toPixels(right) || compareText(leftName, rightName))
    .map(([name]) => name)
}
...
function semanticConditions(
  ...
  if (Array.isArray(value)) {
    ...
    return value.flatMap((item, index) => isPrimitive(item) && names[index] ? [[names[index], item]] : [])
  }
  if (!isPlainObject(value)) return []
  return sortConditions(Object.entries(value).filter((entry): entry is [string, TokenPrimitive] => isPrimitive(entry[1])))
}
...
function walkTokens(
  ...
  for (const key of Object.keys(source).filter((item) => !reservedKeys.has(item)).sort(compareText)) {
...
function walkSemanticTokens(
  ...
  for (const key of Object.keys(source).filter((item) => !reservedKeys.has(item)).sort(compareText)) {
    ...
    if (isPlainObject(value)) walkSemanticTokens(value, nextPath, breakpoints, output)
  }
}

/** Flattens token trees in stable name/condition order. */
export function flattenTokens(options: Pick<TokenDictionaryOptions, 'tokens' | 'semanticTokens' | 'breakpoints'>): FlattenedToken[] {
...
function breakpointTokens(breakpoints: Readonly<Record<string, string>> | undefined): FlattenedToken[] {
  if (!breakpoints) return []
  return Object.entries(breakpoints).flatMap(([name, value]) => [
    {
...
export function createTokenDictionary(options: TokenDictionaryOptions = {}): TokenDictionary {
  ...
  const formatTokenName = (path: string | readonly string[]): string => typeof path === 'string' ? path : tokenName(path)
  const formatCssVar = (path: string | readonly string[], prefixOverride?: string): TokenCssVar => {
    const name = formatTokenName(path)
    const suffix = name.split('.').map(normalizeSegment).filter(Boolean).join('-')
  ...
  const getVar = (path: string | readonly string[], fallback?: string): string | undefined => {
    const name = formatTokenName(path)
  ...
  const expandReferenceInValue = (value: string): string => value.replace(referencePattern, (match, reference: string) => {
    return getVar(reference.trim()) ?? match
  })

  const registerToken = (token: Token): void => {
    const condition = token.extensions.condition ?? 'base'
    const existingIndex = allTokens.findIndex((item) => item.name === token.name && (item.extensions.condition ?? 'base') === condition)
    if (existingIndex >= 0) allTokens.splice(existingIndex, 1, token)
    else allTokens.push(token)
    allTokens.sort((left, right) => compareText(left.name, right.name) || compareConditions(left.extensions.condition, right.extensions.condition))
    ...
    const categoryTokens = categoryMap.get(token.extensions.category) ?? new Map<string, Token>()
    if (!categoryTokens.has(token.name) || condition === 'base') categoryTokens.set(token.name, token)
    categoryMap.set(token.extensions.category, categoryTokens)

    if (token.extensions.category === 'colors') {
      const palette = token.path[1]
      if (palette) {
        const paletteTokens = colorPaletteMap.get(palette) ?? new Map<string, string>()
  ...
  const flattened = [...flattenTokens(options), ...breakpointTokens(options.breakpoints)]
    .sort((left, right) => compareText(left.name, right.name) || compareConditions(left.condition, right.condition))
  ...
  for (const item of flattened) {
    ...
    registerToken({
      ...
      value: typeof item.value === 'string' ? expandReferenceInValue(item.value) : item.value,
      extensions: {
  ...
  const getCategoryValues = (category: string): Record<string, string> => {
    ...
    const entries = Array.from(categoryMap.get(category)?.entries() ?? []).sort(([left], [right]) => compareText(left, right))
    for (const [name, token] of entries) {
      const relativeName = name.startsWith(`${category}.`) ? name.slice(category.length + 1) : name
