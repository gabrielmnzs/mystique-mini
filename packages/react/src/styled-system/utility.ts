...
export function createUtility(options: UtilityOptions): Utility {
  ...
  const resolveToken = (definition: UtilityDefinition, raw: CssValue): CssValue => {
  ...
  return Object.freeze({
    ...
    transform(name: string, raw: CssValue) {
      ...
      if (!definition) {
        const value = name.startsWith('--') && typeof raw === 'string'
          ? token(raw, raw)
          : raw
        return { [name]: value }
      }
      ...
      if (definition.transform) {
        ...
        return important
          ? Object.fromEntries(Object.entries(transformed).map(([key, value]) => [key, `${value} !important`]))
          : transformed
      }
      ...
      return Object.fromEntries(properties.map((property) => [property, resolved]))
    },
  })
}
