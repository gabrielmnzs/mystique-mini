...
export type ResponsiveInput<T> = T | readonly (T | null | undefined)[] | Readonly<Record<string, T | null | undefined>>
...
function parseLength(value: string): { pixels: number; rem: string } {
  ...
  const normalized = Number.isInteger(remValue) ? String(remValue) : String(Number(remValue.toFixed(5)))
  return { pixels: unit === 'px' ? amount : amount * 16, rem: `${normalized}rem` }
}
...
export function createBreakpoints(input: Readonly<Record<string, string>>): Breakpoints {
  const values = Object.entries(input)
    .filter(([name]) => name !== 'base')
    .map(([name, value]) => ({ name, ...parseLength(value) }))
    .sort((left, right) => left.pixels - right.pixels || compareText(left.name, right.name))
    .map<BreakpointEntry>((entry, index, entries) => ({
      max: entries[index + 1] ? subtractAdjustment(entries[index + 1].rem) : undefined,
      min: entry.rem,
      name: entry.name,
    }))
  ...
  const down = (name: string) => mediaQuery({ max: subtractAdjustment(requireEntry(name).min) })
  const only = (name: string) => {
    const entry = requireEntry(name)
    return mediaQuery({ min: entry.min, max: entry.max })
  }
  const between = (from: string, to: string) => mediaQuery({
  ...
  for (const entry of values) {
    ...
    for (const target of values) {
      if (target.min === entry.min || parseLength(target.min).pixels <= parseLength(entry.min).pixels) continue
      conditions[`${entry.name}To${capitalize(target.name)}`] = between(entry.name, target.name)
    }
  }
  ...
  const normalize = <T>(value: ResponsiveInput<T>): Record<string, T> => {
    if (Array.isArray(value)) {
      ...
      for (let index = 0; index < value.length && index < names.length; index += 1) {
    ...
    if (value !== null && typeof value === 'object') {
      ...
      const orderedNames = [
        ...keys(),
        ...Object.keys(source).filter((name) => !keys().includes(name)).sort(compareText),
      ]
      const result: Record<string, T> = {}
