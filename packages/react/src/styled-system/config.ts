...
export function defineSlotRecipe<const T extends SlotRecipeDefinition>(recipe: T): T {
  return recipe
}

export function defineTokens<const T extends NonNullable<ThemingConfig['tokens']>>(tokens: T): T {
  return tokens
}

export function defineSemanticTokens<const T extends NonNullable<ThemingConfig['semanticTokens']>>(tokens: T): T {
  return tokens
}
...
type ValidateSystemStyle<T> = T extends Record<string, unknown>
  ? {
      [K in keyof T]: K extends keyof SystemProperties
        ? T[K] extends SystemProperties[K] ? T[K] : never
        : K extends keyof ConditionProperties
          ? ValidateSystemStyle<NonNullable<T[K]>>
        : K extends `--${string}`
          ? T[K] extends ConditionalValue<CssValue> | undefined ? T[K] : never
...
type ValidateFontFace<T> = T extends Record<string, unknown>
  ? {
      [K in keyof T]: K extends keyof FontFaceDescriptors
        ? T[K] extends FontFaceDescriptors[K] ? T[K] : never
        : never
    }
  : never

type ValidateKeyframes<T> = T extends Record<string, unknown>
  ? { [K in keyof T]: ValidateSystemStyle<T[K]> }
  : never

type ValidateGlobalStyles<T> = T extends Record<string, unknown>
  ? {
...
export function defineUtility<const T extends UtilityDefinition>(utility: T): T {
  return utility
}
