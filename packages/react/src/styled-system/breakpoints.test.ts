...
describe('breakpoints', () => {
  const breakpoints = createBreakpoints({ lg: '992px', sm: '30em', md: '48rem' })
  ...
  it('builds min, max, only, and named range conditions', () => {
    ...
    expect(breakpoints.down('md')).toBe('@media screen and (max-width: 47.9975rem)')
    ...
    expect(breakpoints.getCondition('sm')).toBe('@media screen and (min-width: 30rem)')
    expect(breakpoints.getCondition('smDown')).toBe('@media screen and (max-width: 29.9975rem)')
  ...
  it('normalizes sparse arrays, objects, and scalar values', () => {
    expect(breakpoints.normalize(['base', null, 'medium', 'large', 'ignored'])).toEqual({
      base: 'base',
      md: 'medium',
      lg: 'large',
    })
    expect(breakpoints.normalize({ print: 'print', lg: 'large', base: 'base', sm: null, md: 'medium' })).toEqual({
      base: 'base',
      md: 'medium',
      lg: 'large',
      print: 'print',
  ...
  it('throws a clear error for invalid values and unknown names', () => {
    expect(() => createBreakpoints({ fluid: 'clamp(30rem, 50vw, 60rem)' })).toThrow('Invalid breakpoint value')
    expect(() => breakpoints.only('missing')).toThrow('Unknown breakpoint: missing')
  })
})
