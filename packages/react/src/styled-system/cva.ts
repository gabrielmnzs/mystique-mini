...
function selectedStyle(
  ...
  if (!variants || (typeof value !== 'string' && typeof value !== 'boolean')) return
...
export function createCva(
  ...
  return function cva<
    const T extends RecipeDefinition = RecipeDefinition,
  >(definition: T = {} as T): CvaFn<RecipeVariantProps<T>> {
    const variants = definition.variants ?? {}
    const variantKeys = Object.freeze(Object.keys(variants))
    const variantMap = Object.freeze(
      Object.fromEntries(
        Object.entries(variants).map(([key, values]) => [key, Object.freeze(Object.keys(values))]),
      ),
    )
