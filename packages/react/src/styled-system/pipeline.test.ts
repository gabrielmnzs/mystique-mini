...
describe('styled-system pipeline', () => {
  it('normalizes and serializes responsive values inside conditions, selectors, and at-rules', () => {
    expect(system.normalizeValue(['base', null, 'medium', 'large', 'ignored'])).toEqual({
      base: 'base',
      md: 'medium',
      lg: 'large',
    })
  ...
  it('orders conditions and breakpoints by system order rather than input order', () => {
    ...
    expect(Object.keys(output)).toEqual(['color', media.sm, media.md, media.lg])
    ...
    expect(mixedCustomAliases).toEqual(system.css({
    ...
    const nestedLayeredConditions = system.css(
      { '& .child': { md: { color: 'surface' } } } as unknown as SystemStyleObject,
      { '& .child': { sm: { color: 'brand' } } } as unknown as SystemStyleObject,
    )
    expect(Object.keys(
    ...
    expect(Object.keys(
      nestedAliasConditions['&[data-same-condition]'] as Record<string, unknown>,
    )).toEqual([media.sm, media.md])
  ...
  it('expands token references and emits base and conditional semantic variables', () => {
    ...
    expect(system.tokens.expandReferenceInValue('1px solid {colors.surface}')).toBe(
      '1px solid var(--mystique-colors-surface)',
    )
    expect(system.token('colors.surface')).toBe('var(--mystique-colors-surface)')
    ...
    expect(system.tokens.cssVars.get('_dark')?.get('--mystique-colors-surface')).toBe(
      'var(--mystique-colors-ink)',
    )
    expect(system.tokens.cssVars.get('md')?.get('--mystique-colors-surface')).toBe(
      'var(--mystique-colors-brand)',
    )

    const tokenCss = system.getTokenCss()
    expect(tokenCss).toContain(':where(.contract-theme){')
    expect(tokenCss).toContain('--mystique-colors-border:color-mix(in srgb, var(--mystique-colors-ink) 25%, transparent);')
    expect(tokenCss).toContain('[data-theme=dark]')
    expect(tokenCss).toContain(`${media.md}{:where(.contract-theme){--mystique-colors-surface:var(--mystique-colors-brand);}}`)
  })
  ...
  it('applies cva defaults, responsive variants, compounds, and prop splitting', () => {
    const recipe = system.cva({
      ...
      compoundVariants: [
        { tone: 'loud', raised: true, css: { border: 'thin', borderColor: 'brand' } },
      ],
    })
    ...
    expect(recipe.splitVariantProps({ tone: 'loud', raised: true, id: 'badge', color: 'brand' })).toEqual([
  ...
  it('resolves sva defaults, responsive variants, compounds, and every declared slot', () => {
    const recipe = system.sva({
      ...
      compoundVariants: [
        { tone: 'loud', bordered: true, css: { root: { borderColor: 'ink' }, label: { fontWeight: 700 } } },
      ],
    })

    expect(recipe()).toEqual({
      root: { '@layer recipes': { display: 'flex', background: 'var(--mystique-colors-paper)' } },
  ...
  it('emits declared cascade layers and supports disabling layers and preflight independently', () => {
    ...
    const bare = createSystem(defaultBaseConfig, defineConfig({
      disableLayers: true,
      preflight: false,
      cssVarsPrefix: 'bare',
  ...
  it('preserves declaration and nested top-level global at-rules', () => {
    const withAtRules = createSystem(defaultBaseConfig, defineConfig({
      preflight: false,
