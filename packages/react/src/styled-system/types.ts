...
import type { SemanticTokenDefinition, TokenDefinition } from './token-dictionary'
...
export type ConditionalValue<T> =
  | T
  | readonly (T | Nil)[]
  | { readonly [condition: string]: T | Nil }
...
export type SystemStyleObjectInput =
  ...
  | false
  | Nil
  | readonly SystemStyleObjectInput[]
...
export type GlobalCss = Record<
  string,
  SystemStyleObject | GlobalAtRuleObject
>
