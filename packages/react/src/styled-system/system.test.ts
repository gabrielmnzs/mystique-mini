...
describe('createSystem', () => {
  ...
  it('emits stable prefixed variables, semantic conditions, layers, and preflight', () => {
    ...
    expect(output).toContain(
      '--mystique-colors-fg:var(--mystique-colors-gray-900);',
    );
    expect(output).toContain('[data-theme=dark]');
    expect(output).toContain(
      '--mystique-colors-fg:var(--mystique-colors-gray-50);',
    );
    expect(output).toContain('box-sizing:border-box;');
    expect(defaultSystem.token('colors.red.500')).toBe(
      'var(--mystique-colors-red-500)',
    );
  });
    ...
    const system = createSystem(
      ...
      defineConfig({
        ...
        theme: {
          ...
          semanticTokens: {
            colors: {
              action: { value: { base: '{colors.brand}', md: '#654321' } },
            },
          },
        },
      }),
  ...
  it('resolves cva and sva recipes with defaults, compounds, and slots', () => {
    const recipe = defaultSystem.cva({
      ...
      compoundVariants: [
        { tone: 'loud', raised: true, css: { fontWeight: 'bold' } },
      ],
    });
