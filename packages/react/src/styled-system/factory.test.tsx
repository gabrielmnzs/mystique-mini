...
function renderWithSystem(node: React.ReactNode) {
  return render(
    <MystiqueProvider value={testSystem}>{node}</MystiqueProvider>,
  )
}
...
describe('mystique factory', () => {
  ...
  it('filters against the final `as` target and maps native aliases', () => {
    ...
    const RuntimeIframe = mystique.iframe as ComponentType<Record<string, unknown>>
    const RuntimeObject = mystique.object as ComponentType<Record<string, unknown>>
    ...
    renderWithSystem(
      <>
        ...
        <Image data-testid="image" alt="sample" htmlWidth={40} htmlHeight={30} />
        ...
        <RuntimeInput
          ...
          type="submit"
        />
        <RuntimeImage as="div" alt="not-forwarded" src="/image.png" data-testid="image-as-div" />
        <RuntimeAnchor
    ...
    expect(screen.getByTestId('input-as-button')).not.toHaveAttribute('checked')
    expect(screen.getByTestId('input-as-button')).not.toHaveAttribute('defaultChecked')
    expect(screen.getByTestId('input-as-button')).toHaveAttribute('formaction', '/submit')
    ...
    expect(screen.getByTestId('anchor-metadata')).toHaveAttribute('ping', '/audit')
    expect(screen.getByTestId('anchor-metadata')).toHaveAttribute('type', 'text/html')
    expect(screen.getByTestId('details-metadata')).toHaveAttribute('name', 'accordion')
  ...
  it('filters parent and child props against the final asChild target', () => {
    ...
    renderWithSystem(
      <RuntimeForm asChild action="/parent" method="post">
        <div
          {...({ action: '/child', name: 'child-form' } as Record<string, unknown>)}
  ...
  it('keeps React 19 element metadata target-aware, including native aliases', () => {
    ...
    const RuntimeDialog = mystique.dialog as ComponentType<Record<string, unknown>>
    ...
    const RuntimeMeter = mystique.meter as ComponentType<Record<string, unknown>>
    const RuntimeObject = mystique.object as ComponentType<Record<string, unknown>>
    const RuntimeSlot = mystique.slot as ComponentType<Record<string, unknown>>
    const RuntimeTable = mystique.table as ComponentType<Record<string, unknown>>
    const RuntimeTextarea = mystique.textarea as ComponentType<Record<string, unknown>>
    ...
    expect(screen.getByTestId('metadata-dialog')).toHaveAttribute('closedby', 'any')
    expect(screen.getByTestId('metadata-textarea')).toHaveAttribute('dirname', 'direction')
    expect(screen.getByTestId('metadata-textarea')).toHaveAttribute('wrap', 'soft')
    expect(screen.getByTestId('metadata-meter')).toHaveAttribute('high', '8')
    expect(screen.getByTestId('metadata-slot')).toHaveAttribute('name', 'named-slot')
    expect(screen.getByTestId('metadata-area')).toHaveAttribute('coords', '0,0,10,10')
    expect(screen.getByTestId('metadata-area')).toHaveAttribute('shape', 'rect')
    expect(screen.getByTestId('metadata-object')).toHaveAttribute('classid', 'fixture')
    expect(screen.getByTestId('metadata-object')).toHaveAttribute('wmode', 'opaque')
    expect(screen.getByTestId('metadata-table')).toHaveAttribute('align', 'center')
    expect(screen.getByTestId('metadata-table')).toHaveAttribute('border', '1')
    expect(screen.getByTestId('metadata-table')).toHaveAttribute('width', '100')

    const div = screen.getByTestId('metadata-div')
    for (const attribute of [
      'classid', 'closedby', 'coords', 'high', 'mediagroup', 'shape',
    ]) {
  ...
  it('honors React 19 callback-ref cleanup through asChild composition', () => {
    ...
    const rendered = renderWithSystem(
      <Div asChild ref={outerRef}>
        <a href="/cleanup" ref={childRef}>cleanup</a>
      </Div>,
    )
  ...
  it('preserves custom target requirements and rejects unknown intrinsic props', () => {
    ...
    type PropsAreNotAny = AssertFalse<
      IsAny<ComponentProps<typeof Div>>
    >
