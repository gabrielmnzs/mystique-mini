...
export function createLayers(
  ...
  // Array.from remains iterable-safe when a consumer transpiles this package
  // with a loose array-spread transform (for example, Docusaurus/Webpack).
  const unique = Object.freeze(Array.from(new Set(names)));
  return Object.freeze({
    ...
    wrapObject(name: string, styles: Record<string, unknown>) {
      if (Object.keys(styles).length === 0 || !unique.includes(name))
        return styles;
