...
function mergeContextProps(...sources: Array<AnyProps | undefined>): AnyProps {
...
>(options: SlotRecipeContextOptions<K, R>) {
  ...
  function useRecipeResult(inputProps: AnyProps) {
    ...
    const classNames = runtimeRecipe.className
      ? Object.fromEntries(
          runtimeRecipe.slots.map((slot) => [
            slot,
            `${runtimeRecipe.className}__${slot}`,
          ]),
        )
      : {};
  ...
  const withProvider = <
    ...
    P extends object = SlotRecipeContextProps<K, R>,
  >(
    Component: T,
    slot: SlotRecipeContextSlot<K, R>,
    providerOptions: WithProviderOptions<P> = {},
  ): MystiqueComponent<T, P> => {
    const { defaultProps, displayName, wrapElement, ...factoryOptions } =
      providerOptions;
    ...
    const SlotProvider = React.forwardRef<React.ComponentRef<T>, P>(
      (inputProps, ref) => {
        const propsContext = usePropsContext();
        const props = React.useMemo(
          () =>
    ...
    SlotProvider.displayName =
      displayName ?? `${contextName}${upperFirst(slot)}`;
  ...
  >(
    ...
    const SlotContext = React.forwardRef<React.ComponentRef<T>, P>(
      (inputProps, ref) => {
        ...
        return (
          <SuperComponent
            ...
            className={joinClassNames(
              className,
              slot ? classNames[slot] : undefined,
            )}
          />
        );
      },
