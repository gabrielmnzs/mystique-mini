...
describe('token dictionary', () => {
  ...
  it('flattens names and conditions deterministically', () => {
    ...
    expect(
      flattened.map((token) => `${token.name}:${token.condition ?? 'token'}`),
    ).toEqual([
      'colors.border:token',
      'colors.fg:base',
      'colors.fg:_dark',
    ...
    expect(flattened.find((token) => token.name === 'spacing.2')?.value).toBe(
      '0.5rem',
    );
  });
  ...
  it('rejects token leaves that omit the required value wrapper', () => {
    expect(() =>
      flattenTokens({
        tokens: { colors: { invalid: '#fff' } } as unknown as NonNullable<
          Parameters<typeof flattenTokens>[0]['tokens']
        >,
      }),
    ...
    expect(() =>
      flattenTokens({
        semanticTokens: {
          ...
          Parameters<typeof flattenTokens>[0]['semanticTokens']
        >,
      }),
    ).toThrow(/token leaves must use \{ value \}/);
  });
  ...
  it('resolves token references to prefixed CSS variable references', () => {
    ...
    expect(dictionary.expandReferenceInValue('1px solid {colors.fg}')).toBe(
      '1px solid var(--mystique-colors-fg)',
    );
    expect(dictionary.expandReferenceInValue('{colors.missing}')).toBe(
      '{colors.missing}',
    );
  ...
  it('exposes base and conditional semantic CSS variables', () => {
    ...
    expect(dictionary.cssVars.get('base')?.get('--mystique-colors-fg')).toBe(
      'var(--mystique-colors-ink)',
    );
    expect(dictionary.cssVars.get('base')?.get('--mystique-colors-focus')).toBe(
      'var(--mystique-colors-border)',
    );
    expect(dictionary.cssVars.get('base')?.get('--mystique-spacing-2')).toBe(
      '0.5rem',
    );
    expect(dictionary.cssVars.get('_dark')).toEqual(
      new Map([['--mystique-colors-fg', 'var(--mystique-colors-paper)']]),
    );
    expect(dictionary.getByName('colors.fg')?.extensions.condition).toBe(
      'base',
    );
  ...
  it('registers breakpoints in both canonical token categories', () => {
    ...
    expect(
      dictionary.getByName('sizes.breakpoint-md')?.extensions.cssVar.ref,
    ).toBe('var(--custom-sizes-breakpoint-md)');
  });
});
