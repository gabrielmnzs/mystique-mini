...
import { createBreakpoints, type Breakpoints } from './breakpoints'
import { createConditions, type Conditions } from './conditions'
import { createCva } from './cva'
import { createLayers, serializeCssRule, serializeGlobalCss, type Layers } from './layers'
import { mergeConfigs } from './merge-config'
import { createNormalize, type NormalizeFn } from './normalize'
import { defaultPreflight } from './preflight'
import { createSerialize, type SerializeFn } from './serialize'
...
import { createUtility, type Utility } from './utility'
...
function resolveGlobalStyles(
  ...
  const resolveAtRule = (
    ...
    const onlyDeclarations = entries.every(([, item]) =>
      item === null ||
      item === undefined ||
      typeof item === 'string' ||
      typeof item === 'number',
...
function conditionVariablesRule(
  ...
  if (resolved.startsWith('@')) return `${resolved}{${variablesRule(root, values)}}`
...
export function createSystem(...inputConfigs: readonly SystemConfig[]): SystemContext {
  ...
  const normalizeValue = createNormalize({ breakpointKeys: breakpoints.keys().slice(1) })
  const serialize = createSerialize({ conditions, normalize: normalizeValue, utility })
  ...
  const preflightSource = config.preflight === false
    ? undefined
  ...
  const isValidProperty = (name: string) => {
    return name === 'css' || name.startsWith('--') || utility.has(name) || conditions.has(name)
  }
  ...
  const context: SystemContext = {
    $$mystique: true,
    _config: config,
    _global: [resolvedPreflight as Record<string, SystemStyleObject>, resolvedGlobal as Record<string, SystemStyleObject>],
    ...
    isSlotRecipe: (key) => Object.prototype.hasOwnProperty.call(slotRecipes, key),
    hasRecipe: (key) =>
      Object.prototype.hasOwnProperty.call(recipes, key) ||
      Object.prototype.hasOwnProperty.call(slotRecipes, key),
    hasSlotRecipe: (key) => Object.prototype.hasOwnProperty.call(slotRecipes, key),
  }
