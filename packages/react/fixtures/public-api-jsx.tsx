...
import { createRef, forwardRef, type ComponentProps, type ComponentRef } from 'react'
...
const BaseToneCard = mystique('article', defineRecipe({
  variants: { tone: { base: { opacity: 0.6 } } },
}))
const ExtendedToneCard = mystique(BaseToneCard, defineRecipe({
  variants: { tone: { outer: { opacity: 1 } } },
}))
const compiledMoodRecipe = cva(defineRecipe({
  variants: {
    mood: {
      calm: { opacity: 0.8 },
      excited: { opacity: 1 },
...
const compiledStateRecipe = cva(defineRecipe({
  variants: {
    state: {
      true: { opacity: 1 },
      false: { opacity: 0.5 },
...
const compiledSlotRecipe = sva(defineSlotRecipe({
  slots: ['root'],
  variants: {
    state: {
      true: { root: { opacity: 1 } },
...
>('div')
const Custom = mystique(forwardRef<HTMLDivElement, { required: string }>(
  ({ required, ...props }, ref) => <div ref={ref} {...props}>{required}</div>,
))
const CustomTarget = forwardRef<HTMLSpanElement, { customRequired: string }>(
  ({ customRequired, ...props }, ref) => <span ref={ref} {...props}>{customRequired}</span>,
)
...
const valid = <>
  ...
  <Div as={CustomTarget} customRequired="yes" ref={createRef<HTMLSpanElement>()} />
  ...
  <Custom asChild><button type="button">custom base replaced</button></Custom>
  ...
  <RequiredBehavior asChild requiredBehavior="active"><span /></RequiredBehavior>
  ...
  <Circle as="button" type="button" size="sm" ref={createRef<HTMLButtonElement>()} />
</>
...
const [compiledVariantProps, compiledElementProps] =
  compiledMoodRecipe.splitVariantProps({ mood: 'calm', id: 'compiled' } as const)
...
const namedCompiledSplit = compiledMoodRecipe.splitVariantProps(namedCompiledProps)
const [, erasedRecipeRest] = erasedRecipe.splitVariantProps({ mood: 'calm', id: 'erased' })
const [, erasedSlotRecipeRest] = erasedSlotRecipe.splitVariantProps({ state: true, id: 'erased-slot' })
void [
  valid,
  anchorProps,
  anchorRef,
...
const invalidCompiledInheritedVariant = <CompiledExtendedCard density="missing" />
...
const invalidAsAndAsChild = <Div as="a" asChild><span /></Div>
// @ts-expect-error asChild preserves required behavioral props from the wrapper
const invalidRequiredBehavior = <RequiredBehavior asChild><span /></RequiredBehavior>
// @ts-expect-error custom selected targets keep their required props
const invalidSelectedCustom = <Div as={CustomTarget} />
void [invalidHref, invalidOverriddenHref, invalidRequired, invalidRef, invalidNativeSize, invalidUnknown, invalidVariant, invalidInheritedVariant, invalidExtendedVariant, invalidRewrappedVariant, invalidMergedVariant, invalidCompiledVariant, invalidCompiledInheritedVariant, invalidErasedRecipeProp, invalidErasedRecipeRest, invalidErasedSlotRecipeRest, invalidCompiledCall, invalidCompiledSplit, invalidCompiledBoolean, invalidCompiledSlotCall, invalidCompiledSlotSplit, invalidAsChild, invalidAsAndAsChild, invalidRequiredBehavior, invalidSelectedCustom]
