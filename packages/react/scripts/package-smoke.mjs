...
const packageName = 'mystique-mini-react';
...
function collectExportTargets(value, output = []) {
  if (typeof value === 'string') output.push(value);
    ...
    for (const nested of Object.values(value))
      collectExportTargets(nested, output);
  }
  return output;
}
...
async function validatePublicPackage(packageRoot, expectedVersion) {
  ...
  assert.notEqual(
    manifest.private,
    true,
    'published manifest must not be private',
  );
  ...
  for (const key of Object.keys(manifest.exports)) {
    assert.equal(
      key.includes('*'),
      false,
      `wildcard export is forbidden: ${key}`,
    );
  }
  for (const target of exportTargets) {
    assert.equal(
      target.includes('*'),
      false,
      `wildcard export target is forbidden: ${target}`,
    );
    assert.ok(
      target.startsWith('./'),
      `export target must be package-relative: ${target}`,
    );
    assert.equal(
...
async function writeConsumerFiles(consumerDirectory, tarballPath) {
  ...
  assert.match(
    versions.react,
    /^19\./,
    'the offline smoke requires React 19 in the store',
  );
  assert.match(
    versions.reactDom,
    /^19\./,
    'the offline smoke requires React DOM 19 in the store',
  );
  assert.match(
    versions.reactTypes,
    /^19\./,
    'the offline smoke requires React 19 types',
  );
  assert.match(
    versions.reactDomTypes,
    /^19\./,
    'the offline smoke requires React DOM 19 types',
  );
...
async function main() {
  ...
  const sourceManifest = await readJson(join(packageDirectory, 'package.json'));
  assert.equal(sourceManifest.name, packageName);
  const temporaryRoot = await mkdtemp(
    join(tmpdir(), 'mystique-package-smoke-'),
  );
  ...
  try {
    ...
    runPnpm(pnpmEntrypoint, ['pack', '--pack-destination', packDirectory], {
      cwd: packageDirectory,
    });
    ...
    assert.equal(
      tarballs.length,
      1,
      `expected one tarball, received: ${tarballs.join(', ')}`,
    );
    ...
    await validatePublicPackage(extractedPackage, sourceManifest.version);
    await validateDirectives(extractedPackage);

    for (const path of packedFiles.filter((name) =>
      /\.(?:[cm]?js|[cm]?ts|json)$/.test(name),
    )) {
    ...
    const installedManifest = await validatePublicPackage(
      installedPackage,
    ...
    run(
      process.execPath,
      [typescriptCli, '--project', 'tsconfig.typegen.json'],
      {
        cwd: consumerDirectory,
      },
    );
