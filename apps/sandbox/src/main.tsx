    ...
    <MystiqueProvider theme={theme}>
      <Box as="main" className="site-shell">
        <Flex as="header" className="topbar" align="center" justify="space-between">
          ...
          <Flex as="nav" className="nav-links" gap="1.5rem" aria-label="Primary navigation"><Box as="a" href="#catalog">Catalog</Box><Box as="a" href="#setup">Setup</Box><Box as="a" href="https://github.com" target="_blank" rel="noreferrer">GitHub ↗</Box></Flex>
        </Flex>
        ...
        <Box as="section" className="hero" aria-labelledby="hero-title">
          ...
          <Flex className="hero-bottom" align="end" justify="space-between" gap="2rem">
            <Text className="hero-copy">Mystique Mini is a compact React primitive set for building interfaces that feel considered. Explore the raw pieces, then make them yours.</Text>
        ...
        <Box as="section" id="catalog" className="catalog" aria-labelledby="catalog-title">
          <SectionLabel number="01" children="The catalog" />
          <Flex className="catalog-intro" align="end" justify="space-between" gap="2rem"><Text as="h2" id="catalog-title">Primitives, <em>unvarnished.</em></Text><Text className="intro-note">Seven focused components.<br />One expressive foundation.</Text></Flex>
          ...
          <Box className="specimen-grid">
            ...
            <Box className="specimen" variant="quiet" recipeSize="sm"><Span className="eyebrow">Center / Square / Circle</Span><Flex className="shape-stage" align="center" justify="center" gap="0.75rem"><Center className="center-demo"><Circle boxSize="2.3rem" bg="coral" /></Center><Square boxSize="3.5rem" bg="ink" rounded="sm" /><Circle boxSize="3.5rem" bg="moss" /></Flex><Text className="caption">Geometry with a point of view.</Text></Box>
            <Box className="specimen" variant="quiet" recipeSize="sm"><Span className="eyebrow">Text / Span</Span><Text className="type-demo">Make it <Span color="coral">legible.</Span></Text><Text className="caption">Type hierarchy without ceremony.</Text></Box>
            <Box className="specimen specimen-polymorph" variant="featured"><Span className="eyebrow">Polymorphic Box</Span><Text as="h3">One primitive,<br /><em>many voices.</em></Text><Flex gap="0.65rem" wrap="wrap" mt="1.5rem"><Box as="button" className="sample-control" _hover={{ bg: 'coral', color: 'ink' }} _active={{ transform: 'translateY(2px)' }}>as button</Box><Box as="a" href="#setup" className="sample-control" _hover={{ bg: 'paper', color: 'ink' }} _focus={{ borderColor: 'coral' }}>as link ↗</Box></Flex></Box>
          </Box>
        </Box>

        <Box as="section" className="recipe-section" aria-labelledby="recipe-title"><SectionLabel number="02" children="Recipe card" /><Flex className="recipe-layout" gap="clamp(2rem, 7vw, 7rem)" align="center"><Box flex="1"><Text as="h2" id="recipe-title">Tokens in,<br /><em>character out.</em></Text><Text className="body-copy">Theme configuration stays close to the surface. Add your palette, shape a component recipe, and let the primitives carry the rhythm.</Text></Box><Box className="recipe-preview" variant="featured" recipeSize="lg"><Span className="eyebrow">Specimen / featured</Span><Text as="h3">A custom recipe</Text><Flex align="center" gap="0.75rem" mt="2rem"><Circle boxSize="2.5rem" bg="coral" /><Box><Text fontWeight="bold">Default props</Text><Text className="muted">variant: quiet · size: lg</Text></Box></Flex></Box></Flex></Box>

        <Box as="section" id="setup" className="setup-section" aria-labelledby="setup-title"><SectionLabel number="03" children="Start here" /><Flex className="setup-layout" gap="clamp(2rem, 7vw, 7rem)"><Box flex="1"><Text as="h2" id="setup-title">Bring your<br /><em>own atmosphere.</em></Text><Text className="body-copy">For an existing React 19 app. Node 24 and pnpm 11 keep the baseline pleasantly modern.</Text><Text className="reset-note">Provider resetCSS: on by default.</Text><Box as="button" className="copy-install" onClick={copyInstall}>{copied ? 'Copied to clipboard ✓' : 'Copy install command'}<Span>↗</Span></Box></Box><Box flex="1" className="code-column"><Box className="code-block"><Flex justify="space-between" className="code-head"><Span>INSTALL</Span><Span>pnpm 11 · node 24</Span></Flex><Text as="code">{installCode}</Text></Box><Box className="code-block"><Flex justify="space-between" className="code-head"><Span>USAGE</Span><Span>React 19</Span></Flex><Text as="pre">{usageCode}</Text></Box></Box></Flex></Box>
        <Flex as="footer" className="footer" justify="space-between" align="center"><Text>© 2026 Mystique Mini</Text><Text>Made for the in-between.</Text></Flex>
      </Box>
    </MystiqueProvider>
  )
}
