...
const installButton = () =>
  screen.getByRole('button', {
    name: /copy install command|copied to clipboard/i,
  });
...
describe('install command clipboard feedback', () => {
  ...
  it('injects the API 0.2 system, renders all seven components, and preserves polymorphic focus', () => {
    ...
    expect(sandboxSystem.token('colors.accent')).toBe(
      'var(--mystique-colors-accent)',
    );
    expect(
      sandboxSystem.tokens.cssVars.get('base')?.get('--mystique-colors-accent'),
    ).toBe('var(--mystique-colors-coral)');
    ...
    expect(screen.getByText(/provider contract:/i)).toHaveTextContent(
      'value={system}',
    );
