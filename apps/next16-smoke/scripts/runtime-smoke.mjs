...
const fixture = 'next16-smoke'
const expectedVersion = '16.3.4'
const port = 4316
...
function assertMystiqueSsr(html, marker, label) {
  invariant(html.includes(`data-smoke="${marker}"`), `${label} SSR marker is missing`)
  invariant(html.includes('mystique-box'), `${label} did not render a Mystique component class`)
  const styles = emotionStyleTags(html)
  invariant(styles.length > 0, `${label} did not emit Emotion style tags with the mystique key`)
  invariant(
...
async function waitForManifest(server) {
  ...
  while (Date.now() < deadline) {
    invariant(server.exitCode === null, `next start exited early\n${output.join('')}`)
    try {
...
async function getHtml(pathname) {
  const response = await globalThis.fetch(`${baseUrl}${pathname}`, {
    cache: 'no-store',
    headers: { 'accept-encoding': 'identity', 'user-agent': 'mystique-runtime-smoke' },
  })
...
async function readStreamingRoute() {
  ...
  while (true) {
    ...
    if (done) break
    if (value.byteLength > 0) chunks.push(decoder.decode(value, { stream: true }))
  }
  ...
  chunks.forEach((chunk, index) => {
    streamed += chunk
    if (fallbackChunk < 0 && streamed.includes('data-smoke="streaming-fallback"')) fallbackChunk = index
    if (resolvedChunk < 0 && streamed.includes('data-smoke="streaming-resolved"')) resolvedChunk = index
  })
  invariant(chunks.filter(Boolean).length >= 2, 'Suspense response arrived in fewer than two chunks')
  invariant(fallbackIndex >= 0, 'streaming fallback marker is missing')
  invariant(resolvedIndex > fallbackIndex, 'resolved marker did not arrive after the fallback')
  invariant(resolvedChunk > fallbackChunk, 'fallback and resolved segment were not streamed separately')
...
try {
  ...
  invariant(manifest.fixture === fixture, 'API manifest fixture does not match')
  invariant(manifest.framework?.version === expectedVersion, 'API manifest version does not match')
  invariant(manifest.framework?.bundler === 'webpack', 'API manifest bundler does not match')

  const appHtml = await getHtml('/')
  assertMystiqueSsr(appHtml, 'app-router-ssr', 'App Router')

  const pagesHtml = await getHtml('/pages-smoke')
  const pagesStyles = assertMystiqueSsr(pagesHtml, 'pages-router-ssr', 'Pages Router')
  invariant(
  ...
  globalThis.console.log(JSON.stringify({ fixture, ok: true, streamChunks }, null, 2))
} catch (error) {
