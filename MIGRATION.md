...
system-based architecture inspired by Chakra UI v3. It is intentionally a
...
- two dedicated compatibility fixtures, `apps/next15-smoke` and
  ...
  external consumers;
- no configured Git remote or package-publication workflow; and
- no confirmed external source or package consumers.

The package tarball smoke and the two Next.js fixtures validate distribution
and framework compatibility; they do not add consumers to the inventory. On
the inventory evidence, 0.2.0 does not ship a `/legacy` subpath and does not
...
The context is strict in 0.2. Render styled components under
`MystiqueProvider`; do not rely on an implicit default theme.

## Tokens and semantic tokens

Token leaves now use the Design Tokens Community Group-style `{ value }`
shape. Convert scalar leaves such as:
...
```ts
tokens: { colors: { brand: { value: "#9f3d2d" } } }
```
...
install the required `@emotion/react` peer and mount `MystiqueNextProvider`
from `@gabrielmnzs/mystique-react/next`. The package declares
`@emotion/cache` as a direct dependency. Create or import the system inside a
client module; a created system contains functions and is not a serializable
...
fixture/consumer integration rather than a dependency of the published
Mystique package.

The documented Emotion hydration limitation under Turbopack does not change
the streaming-registry contract. On Next.js 15, Webpack is already the default:
